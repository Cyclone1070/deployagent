...
type MockUI struct {
	...
	ReadPermissionFunc func(ctx context.Context, prompt string, preview *uimodel.ToolPreview) (uimodel.PermissionDecision, error)
	...
	CommandsChan chan uimodel.UICommand
}
...
func NewMockUI() *MockUI {
	return &MockUI{
		...
		CommandsChan: make(chan uimodel.UICommand, 1), // Buffered to prevent blocking in simple tests
	}
}
...
func (m *MockUI) ReadPermission(ctx context.Context, prompt string, preview *uimodel.ToolPreview) (uimodel.PermissionDecision, error) {
	if m.ReadPermissionFunc != nil {
		return m.ReadPermissionFunc(ctx, prompt, preview)
	}
	return uimodel.DecisionAllow, nil
}
...
func (m *MockUI) Commands() <-chan uimodel.UICommand {
