...
// MockFileInfo implements model.FileInfo
...
// Write implements model.FileHandle.Write
...
// Sync implements model.FileHandle.Sync
...
// Close implements model.FileHandle.Close
...
// MockFileSystem implements model.FileSystem with in-memory storage.
...
func (f *MockFileSystem) Stat(path string) (model.FileInfo, error) {
	f.Mu.RLock()
	defer f.Mu.RUnlock()
...
func (f *MockFileSystem) Lstat(path string) (model.FileInfo, error) {
	f.Mu.RLock()
	defer f.Mu.RUnlock()
...
func (f *MockFileSystem) ReadFileRange(path string, offset, limit int64) ([]byte, error) {
	...
	if offset < 0 {
		return nil, model.ErrInvalidOffset
	}
...
func (f *MockFileSystem) CreateTemp(dir, pattern string) (string, model.FileHandle, error) {
	f.Mu.Lock()
	defer f.Mu.Unlock()
...
func (f *MockFileSystem) ListDir(path string) ([]model.FileInfo, error) {
	f.Mu.RLock()
	defer f.Mu.RUnlock()
...
// MockCommandExecutor implements model.CommandExecutor for testing
type MockCommandExecutor struct {
	RunFunc   func(ctx context.Context, cmd []string) ([]byte, error)
	StartFunc func(ctx context.Context, command []string, opts model.ProcessOptions) (model.Process, io.Reader, io.Reader, error)
}
...
func (m *MockCommandExecutor) Start(ctx context.Context, command []string, opts model.ProcessOptions) (model.Process, io.Reader, io.Reader, error) {
	if m.StartFunc != nil {
...
// MockProcess implements model.Process for testing
