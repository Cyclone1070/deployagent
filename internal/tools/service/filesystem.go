...
func (r *OSFileSystem) Stat(path string) (model.FileInfo, error) {
	return os.Stat(path)
}

// Lstat returns file info for a path without following symlinks.
func (r *OSFileSystem) Lstat(path string) (model.FileInfo, error) {
...
func (r *OSFileSystem) CreateTemp(dir, pattern string) (string, model.FileHandle, error) {
...
func (r *OSFileSystem) ListDir(path string) ([]model.FileInfo, error) {
	entries, err := os.ReadDir(path)
	if err != nil {
		return nil, err
	}

	infos := make([]model.FileInfo, 0, len(entries))
