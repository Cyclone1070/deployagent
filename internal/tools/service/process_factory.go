...
// OSProcess wraps an os/exec.Cmd to implement the model.Process interface.
...
// OSCommandExecutor implements model.CommandExecutor using os/exec for real system commands.
...
func (f *OSCommandExecutor) Start(ctx context.Context, command []string, opts model.ProcessOptions) (model.Process, io.Reader, io.Reader, error) {
	if len(command) == 0 {
