...
func Resolve(ctx *model.WorkspaceContext, path string) (abs string, rel string, err error) {
	...
	if err != nil {
		return "", "", model.ErrOutsideWorkspace
	}
...
func resolveRelativePath(ctx *model.WorkspaceContext, relPath string) (string, error) {
	workspaceRootAbs := filepath.Clean(ctx.WorkspaceRoot)
	const maxHops = 64
...
func followSymlinkChain(ctx *model.WorkspaceContext, path string, workspaceRoot string, maxHops int) (resolved string, exists bool, err error) {
	...
	for hopCount := 0; hopCount <= maxHops; hopCount++ {
		...
		if info.Mode()&os.ModeSymlink == 0 {
			...
			if !isWithinWorkspace(current, workspaceRoot) {
				return "", false, model.ErrOutsideWorkspace
			}
