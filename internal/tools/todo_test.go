package tools

import (
	"context"
	"sync"
	"testing"

	"github.com/Cyclone1070/iav/internal/tools/model"
)

func TestTodoTools(t *testing.T) {
	// Helper to create a context with a fresh store
	createContext := func() *model.WorkspaceContext {
		return &model.WorkspaceContext{
			TodoStore: NewInMemoryTodoStore(),
		}
	}

	t.Run("Happy Path", func(t *testing.T) {
		ctx := createContext()

		// 1. Initial Read should be empty
		readResp, err := ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
		if err != nil {
			t.Fatalf("ReadTodos failed: %v", err)
		}
		if len(readResp.Todos) != 0 {
			t.Errorf("expected empty todos, got %d", len(readResp.Todos))
		}

		// 2. Write some todos
		todos := []model.Todo{
			{Description: "Task 1", Status: model.TodoStatusPending},
			{Description: "Task 2", Status: model.TodoStatusInProgress},
		}
		writeResp, err := WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: todos})
		if err != nil {
			t.Fatalf("WriteTodos failed: %v", err)
		}
		if writeResp.Count != 2 {
			t.Errorf("expected count 2, got %d", writeResp.Count)
		}

		// 3. Read back and verify
		readResp, err = ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
		if err != nil {
			t.Fatalf("ReadTodos failed: %v", err)
		}
		if len(readResp.Todos) != 2 {
			t.Fatalf("expected 2 todos, got %d", len(readResp.Todos))
		}
		if readResp.Todos[0].Description != "Task 1" {
			t.Errorf("expected Task 1, got %s", readResp.Todos[0].Description)
		}
		if readResp.Todos[1].Status != model.TodoStatusInProgress {
			t.Errorf("expected InProgress, got %s", readResp.Todos[1].Status)
		}
	})

	t.Run("Overwrite", func(t *testing.T) {
		ctx := createContext()

		// Write List A
		listA := []model.Todo{{Description: "A", Status: model.TodoStatusPending}}
		_, err := WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: listA})
		if err != nil {
			t.Fatalf("WriteTodos A failed: %v", err)
		}

		// Write List B
		listB := []model.Todo{{Description: "B", Status: model.TodoStatusCompleted}}
		_, err = WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: listB})
		if err != nil {
			t.Fatalf("WriteTodos B failed: %v", err)
		}

		// Read should return List B
		readResp, err := ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
		if err != nil {
			t.Fatalf("ReadTodos failed: %v", err)
		}
		if len(readResp.Todos) != 1 {
			t.Fatalf("expected 1 todo, got %d", len(readResp.Todos))
		}
		if readResp.Todos[0].Description != "B" {
			t.Errorf("expected B, got %s", readResp.Todos[0].Description)
		}
	})

	t.Run("Empty Write Clears", func(t *testing.T) {
		ctx := createContext()

		// Write something
		_, _ = WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: []model.Todo{{Description: "Task", Status: model.TodoStatusPending}}})

		// Write empty
		_, err := WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: []model.Todo{}})
		if err != nil {
			t.Fatalf("WriteTodos empty failed: %v", err)
		}

		// Read should be empty
		readResp, err := ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
		if err != nil {
			t.Fatalf("ReadTodos failed: %v", err)
		}
		if len(readResp.Todos) != 0 {
			t.Errorf("expected empty list, got %d items", len(readResp.Todos))
		}
	})

	t.Run("Data Isolation", func(t *testing.T) {
		ctx := createContext()

		// Write initial data
		initial := []model.Todo{{Description: "Original", Status: model.TodoStatusPending}}
		_, _ = WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: initial})

		// Read data
		readResp, _ := ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})

		// Modify returned slice
		readResp.Todos[0].Description = "Modified"

		// Read again - should be original
		readResp2, _ := ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
		if readResp2.Todos[0].Description != "Original" {
			t.Error("ReadTodos returned a reference to internal state, not a copy")
		}
	})

	t.Run("Context Isolation", func(t *testing.T) {
		// Verify that two different contexts have different stores
		ctx1 := createContext()
		ctx2 := createContext()

		// Write to ctx1
		_, err := WriteTodos(context.Background(), ctx1, model.WriteTodosRequest{Todos: []model.Todo{
			{Description: "Ctx1", Status: model.TodoStatusPending},
		}})

		// Read from ctx2 - should be empty
		readResp, err := ReadTodos(context.Background(), ctx2, model.ReadTodosRequest{})
		if err != nil {
			t.Fatalf("ReadTodos ctx2 failed: %v", err)
		}
		if len(readResp.Todos) != 0 {
			t.Errorf("expected ctx2 to be empty, got %d items", len(readResp.Todos))
		}
	})

	t.Run("Concurrency", func(t *testing.T) {
		ctx := createContext()
		var wg sync.WaitGroup

		// Launch 100 goroutines reading and writing
		for i := range 100 {
			wg.Add(1)
			go func(id int) {
				defer wg.Done()
				if id%2 == 0 {
					_, _ = WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: []model.Todo{{Description: "Concurrent", Status: model.TodoStatusPending}}})
				} else {
					_, _ = ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
				}
			}(i)
		}
		wg.Wait()
	})

	t.Run("Missing Store", func(t *testing.T) {
		ctx := &model.WorkspaceContext{} // No store initialized

		_, err := ReadTodos(context.Background(), ctx, model.ReadTodosRequest{})
		if err == nil {
			t.Error("expected error when reading with missing store, got nil")
		}

		_, err = WriteTodos(context.Background(), ctx, model.WriteTodosRequest{Todos: []model.Todo{}})
		if err == nil {
			t.Error("expected error when writing with missing store, got nil")
		}
	})
}
